...
func TestRegisterManifoldAgentRoutesNilBundleIsNoop(t *testing.T) {
	r := chi.NewRouter()
	registerManifoldAgentRoutes(r, nil)
	count := 0
	if err := chi.Walk(r, func(_ string, _ string, _ http.Handler, _ ...func(http.Handler) http.Handler) error {
		count++
		return nil
	}); err != nil {
		t.Fatalf("walk routes: %v", err)
	...
	if count != 0 {
		t.Fatalf("route count = %d, want 0", count)
	}
}
