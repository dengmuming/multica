...
type manifoldAgentRouteBundle struct {
	...
	WorkGraph *loophttp.WorkGraphHandler
...
func buildManifoldAgentRoutes(pool *pgxpool.Pool, queries *db.Queries, h *handler.Handler) *manifoldAgentRouteBundle {
	...
	projection := loopservice.NewMulticaPolicyProjectionReader(loopservice.DBPolicyIssueReader{Queries: queries}, loopState, loopState)
	...
	rawLoopReads := loopservice.NewRawLoopReadRepository(pool)
	loopReads := loopservice.NewLoopReadService(rawLoopReads)
	workGraphs := loopservice.NewMulticaWorkGraphProjectionReader(rawLoopReads, loopState)
	...
	return &manifoldAgentRouteBundle{
		...
		WorkGraph: loophttp.NewWorkGraphHandler(workGraphs),
...
func registerManifoldAgentRoutes(r chi.Router, bundle *manifoldAgentRouteBundle) {
	if r == nil || bundle == nil { return }
	if bundle.Loop != nil { bundle.Loop.Register(r) }
	if bundle.Read != nil { bundle.Read.Register(r) }
	if bundle.WorkGraph != nil { bundle.WorkGraph.Register(r) }
	if bundle.Templates != nil { bundle.Templates.Register(r) }
	if bundle.Artifacts != nil { bundle.Artifacts.Register(r) }
}
